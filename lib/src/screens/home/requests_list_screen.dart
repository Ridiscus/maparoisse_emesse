  ...
  void _showRequestDetails(Map<String, dynamic> request) {
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        ...
        return _RequestDetailModal(request: request);
      },
    );
  }
...
class _RequestDetailModal extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String montant = request['montant_offrande'] ?? '0';
    final String intercesseur = request['interception_par'] ?? 'Non spécifié';
    final String statut = request['statut'] ?? 'inconnu';
    ...
    return Container(
      ...
      // ✅ CORRECTION ICI : Ajout de SafeArea
      // Cela dit à l'app : "Ne dessine pas de contenu important (texte/boutons)
      // là où il y a la barre de navigation système".
      child: SafeArea(
        top: false, // On n'a pas besoin de protéger le haut (status bar) ici
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              _buildStatusBadge(context, statut, paiements),
              ...
              _buildDetailRow(context, Icons.person_search_outlined, "Intercesseur", intercesseur),
              _buildDetailRow(context, Icons.account_balance_wallet_outlined, "Offrande", "$montant FCFA"),

              const SizedBox(height: 24),

              // Boutons d'action (qui seront maintenant remontés au-dessus de la barre)
  ...
  Widget _buildActionButtons(BuildContext context, String statut, Map<String, dynamic> request) {
    ...
    if (statusLower == statusEnAttente) {
      return Row(
        children: [
          Expanded(
          ...
          Expanded(
            child: PrimaryButton(
              ...
              onPressed: () {
                ...
                // 1. Récupère le tableau 'paiements'
                final List<dynamic>? paiements = request['paiements'] as List?;

                if (paiements == null || paiements.isEmpty) {
                  _showError(context, "Erreur: Aucune référence de paiement trouvée pour cette demande.");
                  return;
                }

                // 2. Prend le premier paiement de la liste
                final Map<String, dynamic> paiement = paiements[0];
                final String? reference = paiement['reference'];
                final int messeId = request['id'] as int;



                // 3. Récupère les montants
                final double montantOffrande = double.tryParse(request['montant_offrande'] ?? '0') ?? 0;
                final double montantTotal = double.tryParse(paiement['montant'] ?? '0') ?? 0;
                final double frais = (montantTotal > montantOffrande) ? (montantTotal - montantOffrande) : 0.0;
                ...
                Navigator.pushNamed(
                  ...
                  arguments: {
                    ...
                    'frais': frais,
                    'total': montantTotal,
                    'reference': reference, // On passe la référence
                    'messeId': messeId,
                  },
                );
              },
        ...
        children: [
          ...
          Expanded(
            child: PrimaryButton(
              ...
              onPressed: () {
                // 1. On récupère la hauteur de l'écran pour le calcul
                final double screenHeight = MediaQuery.of(context).size.height;

                // 2. On affiche le SnackBar
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    // Contenu avec une petite icône
                    content: Row(
                      children: [
                    ...
                    behavior: SnackBarBehavior.floating, // Important : détacher du bord
                    shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(12)
                    ),
                    // 3. L'ASTUCE : On met une marge énorme en bas pour le pousser en haut
                    // (Hauteur écran - environ 120px pour laisser la place à la barre de statut)
                ...
                try {
                  ...
                  // On n'attend pas (await) car le service ouvre un nouvel écran
                  pdfService.generateAndShowReceipt(request: request);
                } catch (e) {
                  _showError(context, "Erreur lors de la création du PDF: $e");
                }

                // Appel de ta fonction de génération ici...
                // service.generateAndShowReceipt(...);
              },
              // --- FIN MODIFICATION ---
            ),
          ),
